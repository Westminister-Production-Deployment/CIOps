    ...
    """
        ...
        node(POD_LABEL) {
            try {
                ...
                String REPO_NAME = env.REPO_NAME ? env.REPO_NAME : "docker.io/westminister";         
                            ...
                            container(name: 'kaniko', shell: '/busybox/sh') {
                                ...
                                for(int j=0; j<jobConfig.getBuildConfigs().size(); j++){
                                    ...
                                    sh """
                                        ...
                                        --no-push=${noPushImage} \
                                        --cache-repo=westminister/cache/cache
                                    """  
                ...
                if (currentBuild.result == 'SUCCESS') {
                    slackSend (
                        botUser: true,
                        color: 'good',
                        message: "Docker Image build successful for ${JOB_BASE_NAME} service! :tada:\nRepository: DIGIT-OSS\nBranch: ${env.BRANCH.split('/')[-1]}\nDocker Image pushed to Docker Hub.",
                        channel: '#e-gov',
                        teamDomain: 'westminister',
                        tokenCredentialId: 'slack-bot-token'
                    )
                    emailext body: "Docker image for the ${JOB_BASE_NAME} service has been successfully build! 🎉\nRepository: DIGIT-OSS\nBranch: ${env.BRANCH.split('/')[-1]}\nDocker Image pushed to Docker Hub.", subject: "Docker Image Build and Push Successful: ${JOB_BASE_NAME}", to: 'mannan.siddiqui@ongraph.com,cc:md.danish@ongraph.com'
                } else {
                    slackSend (
                        botUser: true,
                        color: 'danger',
                        message: "Docker Image build failed for ${JOB_BASE_NAME} service! :disappointed:\nRepository: DIGIT-OSS\nBranch: ${env.BRANCH.split('/')[-1]}\nPlease refer to this URL for more details: ${BUILD_URL}",
                        channel: '#e-gov',
                        teamDomain: 'westminister',
                        tokenCredentialId: 'slack-bot-token'
                    )
                    emailext body: "Docker image build for the ${JOB_BASE_NAME} service has failed! 😞\nRepository: DIGIT-OSS\nBranch: ${env.BRANCH.split('/')[-1]}\nPlease refer to this URL for more details: ${BUILD_URL}", subject: "Docker Image Build and Push Failed: ${JOB_BASE_NAME}", to: 'mannan.siddiqui@ongraph.com,cc:md.danish@ongraph.com'
                }
            }
        }
    }
